function saterr_imp_save_B(fname,Rad,Noise,WarmLoad,Reflector,MirrorCold,ScanBias,TimeVarying,TBsrc,AP,Orbit,PolOffset,Faraday,Path,tw,tc,cc,cw,cs)
...
if Orbit.onoff==1
    ...
    Orbit.sc.h = single(Orbit.sc.h);
    Orbit.sc.lat = single(Orbit.sc.lat);
    Orbit.sc.lon = single(Orbit.sc.lon);
    Orbit.sc.az = single(Orbit.sc.az);
end
