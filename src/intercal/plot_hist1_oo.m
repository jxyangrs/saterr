...
clf
bar(avg_tar_tb_dif)
xlim([0, size(avg_tar_tb_dif,1)+1])

xlabel('Channel')
ylabel('O-B (K)')
title([str_tar_titlename,' TB O-B (K)']);

outfile = [str_tar_savename,'-hist1-bar_tb_dif.png'];
...
plot(x,y);
xlim([-10 10])
xlabel([str_tar_titlename,' TB Dif (K)']);ylabel('No.');legend(cellstr(num2str(tar_sensor.chanind(:))));legend boxoff
