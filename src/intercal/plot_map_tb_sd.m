...
if 1
    ...
    c_range = avg_tar_tb_sd_std(:,1,ind_node);
    c_range = c_range*2;c_range=[-c_range,c_range];
    c_range = bsxfun(@plus, c_range, avg_tar_tb_sd(:,1,ind_node)); % [channel,upper&lower]
    ...
    for nchan=1: size(map_tar_tb_sd,3)
        
        str_chan = num2str(nchan/1e2,'%.2f');
        str_chan = str_chan(3:4);
        z = map_tar_tb_sd(:,:,nchan,ind_node);%zmean = nanmean(z(:));z=z-zmean;
        
        plotstr.titlecolorbar='(K)';
        plotstr.title={[str_tar_titlename,' TB SD ','Chan ',num2str(nchan)]};
        plotstr.savename=[str_tar_savename,'-map-tar_tb_sd_chan',str_chan];
