function [lat,lon,atm_pres1D,atm_t,atm_q,sfc_pres,sfc_u10,sfc_v10,sfc_skt,sfc_sst] = ucar_read_era5_ana_pl_37_sfc_umd(inpath,infile)
% read nc file of era5 of pressure level 37
% 
% Input:
...
%       atm_pres1D,     atm pressure (Pa),              [layer(up-down),1]/[37], 37-layer from 1000 to 1 mbar
...
% read
fin = [inpath,'/',infile];
lat = double(ncread(fin,'lat'));
lon = double(ncread(fin,'lon'));
atm_pres1D = double(ncread(fin,'atm_pres1D'));
