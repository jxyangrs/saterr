...
for nday = 1: size(ndatestr,1)
    ...
    fileID = ['sim_count_',Path.sensor.spacecraft,'_',Path.sensor.name,'*'];
    ...
    fileID = ['cal_tatb_',Path.sensor.spacecraft,'_',Path.sensor.name,'*'];
    files = dir([inpath,'/',fileID]);
    for nfile=1: size(files,1)
        infile = files(nfile).name;
        disp(infile)
        ...
        fov_lat = [fov_lat,double(data.lat)];
        fov_lon = [fov_lon,double(data.lon)];
        sc_time = [sc_time,double(data.time_sc)];
    end
    
end

% outpath
outpath = [Path.cal.plot,'/',ndatestr(1,:),'_',ndatestr(end,:)];
if ~exist(outpath,'dir')
    mkdir(outpath);
end
