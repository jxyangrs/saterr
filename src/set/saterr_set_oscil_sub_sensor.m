function saterr_set_oscil_sub_sensor
% Orbit variation for sensors
% 
% Input:
%         warm-load variation
%         gain variation
...
TimeVarying.oscillation.warmload.source = 1;
...
switch TimeVarying.oscillation.warmload.source
    ...
    case 1
        ...
        TimeVarying.oscillation.warmload.wave.amp = 0.17*ones(Rad.num_chan,1); % amplitude (Kelvin), [channel,1], amp=1 means peak-to-peak value is 2
        TimeVarying.oscillation.warmload.wave.num_period = Rad.num_orbit*ones(Rad.num_chan,1); % number of periods, [channel,1]
        TimeVarying.oscillation.warmload.wave.phase = pi*ones(Rad.num_chan,1); % phase, [channel,1], [-pi,pi], e.g. pi*ones(Rad.num_chan,1)
    ...
    case 2
        ...
        % customize here. e.g.:
        TimeVarying.oscillation.preset.tw_cw_var = 0.5*ones(n1,n2,n3); % [cross-track,along-track,channel]
        TimeVarying.oscillation.preset.tw_PRT_var = mean(TimeVarying.oscillation.preset.tw_cw_var,1); % [1,along-track,channel]
    ...
    case 3
        saterr_set_oscil_sub_sensor_table(Rad.spacecraft,Rad.sensor)
...
switch TimeVarying.oscillation.gain.source
    ...
    case 1
        ...
        TimeVarying.oscillation.gain.wave.amp = [0.34,0.38,2.86,2.66,0.23]; % amplitude (Kelvin), [channel,1], amp=1 means peak-to-peak value is 2
...
switch TimeVarying.oscillation.Tr.source
    ...
    case 1
        ...
        TimeVarying.oscillation.Tr.wave.amp = [1.62,0.92,3.68,4.02,1.35]; % amplitude (Kelvin), [channel,1], amp=1 means peak-to-peak value is 2
    ...
    case 2
        ...
        y = 5*sin(2*pi*1*x+0);
        y = reshape(y,[n1,n2]);
        y = y(:,:,ones(n3,1)); % [cross-track,along-track,channel]
        TimeVarying.oscillation.preset.Tr_var = y;
